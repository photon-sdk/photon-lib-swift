...
class CloudStoreTests: XCTestCase {
    ...
    func testGetKey() {
        ...
        // Given
        XCTAssertNil(cloudStore.getKey(), "should not find item") // Attempt to get the key
        try? cloudStore.putKey(keyId: keyId, ciphertext: ciphertext) // Now put the key
        
        // When
        let checkKey = cloudStore.getKey() // Now get the key just put, and check that it's contents are the same as what we started with
        
        // Then
        XCTAssertEqual(checkKey?.keyId, keyId)
        XCTAssertEqual(checkKey?.ciphertext, ciphertext)
    }
    ...
    func testRemovePhone() {
        XCTAssertNoThrow(try cloudStore.putPhone(userId: phone), "Store Item")
        XCTAssertEqual(cloudStore.getPhone(), phone, "Should get stored item by userId number")
        XCTAssertNoThrow(cloudStore.removePhone())
    ...
    func testRemoveEmail() {
        ...
        XCTAssertEqual(cloudStore.getEmail(), email, "should get stored item by userId number")
        XCTAssertNoThrow(cloudStore.removeEmail())
